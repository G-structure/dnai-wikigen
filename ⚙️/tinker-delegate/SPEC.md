...
2. **Tinker billing settings page** — Browser automation and encrypted card-channel code exist. Local Neko reaches the Stripe Elements payment form, fills the test card, and has returned bounded decline/failure receipts without exposing card details. Add-balance fails closed with `Payment method required before adding balance` when no real card is on file. Payment-method and add-balance automation use selector fallback families for Tinker billing controls and return bounded `selector_missing` receipts when top-up controls cannot be found. Add-balance enforces whole-dollar `TINKER_MIN_ADD_BALANCE_USD` / `TINKER_MAX_ADD_BALANCE_USD` policy before browser launch, returning bounded `policy_denied` receipts for non-finite, non-positive, below-minimum, fractional, or over-cap requests. Tinker's current UI minimum is `$10`, so source/config defaults now use `$10` for both min and validation cap. `add-card-encrypted-prompt` is the intended operator path for approved real-card validation because it prompts interactively instead of placing card fields in command-line arguments and requires deployed compose/app/OS-image attestation expectations unless explicitly run in local-development mode; it can also require live `TinkerAccountEncumbrance` approval with `--require-encumbrance` before prompting. `funding-validation-packet --prompt-card --run-card-attempt` brings that prompt boundary into the one-command evidence packet flow, rejects missing deployed measurement policy before prompting, and can check the encumbrance for both add-payment-method and same-packet add-balance amount approval before any card material is entered. `funding-command-plan` builds bounded preflight, encumbrance-preflight, and prompt-packet command templates from `deployments/base-sepolia.json`; it references runtime bearer and RPC values only by environment-variable name. The current 2026-07-09 GitHub-attested funding-validation Phala profile is live at attested compose hash `f4728f219572c09c6ccc013883a4a895f3e366ea6e9654459fd6b0002fe33552`; authenticated `/auth/reauth` succeeds and the compose hash is approved by `TinkerAccountEncumbrance`. The first approved real-card validation still did not top up the account: live balance returned `$0.00`; the payment-method receipt reached `payment_submitted` with bounded card-management copy; and add-balance reached the modal but failed closed with `selector_missing` / `Add-balance amount input not found`. Source/tests now treat the card-management copy as payment-method success, add scoped amount-input plus preset-amount fallbacks, expose bounded card-on-file status through `GET /billing/payment-method-status`, and provide authenticated admin/operator removal through `POST /billing/card/remove`. Those status/removal paths return only bounded booleans/bands/receipts, never card brand, last4, expiry, address, or page text. This fix is not Phala-deployed yet, and the deployed encumbrance still has `$5` add-balance/spend caps; update the contract policy to `$10`, rebuild/attest/redeploy the delegate image, approve the new compose hash, then rerun a bounded add-balance attempt. `TINKER_FUNDING_MODE=manual_prefund` remains the default production model and denies encrypted card/add-balance automation before decryption or browser launch; `operator_capped_validation` is required for deliberate capped operator validation. The add-balance HTTP mutation endpoint is additionally disabled by default and requires `TINKER_ALLOW_ADD_BALANCE_ENDPOINT=true`. `docs/TINKER-FUNDING-MODEL.md` and `docs/STRIPE-PCI-FUNDING-SCOPE.md` limit raw-card encrypted delivery to a capped operator-owned validation path; production/repeated funding should use an official Tinker route, Stripe-hosted/tokenized collection, SetupIntent / PaymentMethod reuse with consent, or manual/developer prefunding until compliance review approves otherwise.

3. **TinkerAccountEncumbrance deployment** — The contract, runtime preflight, and no-raw-key Base Sepolia deploy helper exist. The helper defaults the initial compose hash from `deployments/base-sepolia.json`, can use the bounded `oracleEmailHash` as a temporary account commitment, validates caps and bytes32 inputs, runs build/tests, and dry-runs successfully. On 2026-07-09 the helper was re-dry-run against the funding-validation compose hash, passed 9 contract tests, and predicted `0x9F2616f3F7B0dc363bBa19F7d72b9061f791a06e`; this is dry-run evidence only. Actual deployment is still open because Foundry needs an interactive keystore prompt for `--account dev`; non-interactive Codex attempts did not broadcast and chain checks showed unchanged nonce, unchanged balance, and no code at the predicted address. `funding-command-plan` now prints the exact dry-run and broadcast helper commands so the operator terminal handoff is deterministic without putting private keys, keystore passwords, bearer tokens, card details, or RPC secrets into the plan.
