...
PHASE 1: TINKER SIGNUP  LOCAL VALIDATED — deployed CVM validation pending
  ...
  `eb3bde3b5b156dfdd46f701ab2df8f1f19d94120` images, local image-policy hash
  ...
  the required Playwright sidecar digest, and live TDX envelope evidence.
  ...
  required`. `$5` remote preflight with recorded deployment identity is ready,
  but real-card funding is blocked: `funding-command-plan` still returns
  `ready=false` because `TinkerAccountEncumbrance` is not deployed,
  authenticated reauth returns bounded
  `auth_access_blocked` at `auth_email_submitted`, before OTP, and
  authenticated add-balance reaches `billing_page_loaded` but returns bounded
  `auth_required`. Do not run approved real-card material until the encumbrance
  is deployed and the deployed auth/session path is repaired with bounded
  evidence.
