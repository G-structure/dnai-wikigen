...
PHASE 1: TINKER SIGNUP  LOCAL VALIDATED — deployed CVM validation pending
  ...
  PHALA SELECTOR PROBE FINDING: GitHub-attested `ca877db` oracle/delegate images
  were pinned into the one-shot bootstrap profile with only
  `TINKER_ALLOW_SELECTOR_PROBE_ENDPOINT=true` added. The live endpoint returned
  ...
  normal compose afterward, and the same endpoint returned 403. This proves the
  deployed endpoint gate/fail-closed path but not actual selector/frame matches,
  because the browser connection was unavailable.
  ...
  expose raw browser URLs. Normal Phala compose disables the endpoint, and the
  one-shot bootstrap measurement profile enables it for the next deployed probe.
