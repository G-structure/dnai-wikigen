...
PHASE 1: TINKER SIGNUP  LOCAL VALIDATED — deployed CVM validation pending
  ...
  CDP metadata reachability, Playwright/CDP handshake results, and bounded error
  kinds; it does not navigate, click, type, screenshot, inspect page text, or
  expose raw browser URLs. Normal Phala compose disables the endpoint. The
  one-shot bootstrap measurement profile has now proven the route on Phala with
  bounded `cdp_timeout`; the restored normal compose returns 403.
