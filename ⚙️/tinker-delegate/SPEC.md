...
| **How is the Tinker account funded?** | Pre-funded by the developers (us). The encrypted raw-card channel is limited to a capped operator-owned validation path; production/repeated funding should use an official Tinker route, Stripe-hosted/tokenized collection, SetupIntent / PaymentMethod reuse with consent, or manual/developer prefunding until compliance review approves otherwise. |
