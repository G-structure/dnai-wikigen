...
PHASE 1: TINKER SIGNUP  LOCAL VALIDATED — deployed CVM validation pending
  ...
  Source/tests now preserve page target inventory across per-page frame-tree
  timeout by returning page URL class/hash, attach status, frame-tree error
  kind, and an empty frame observation list; this refinement is not yet
  Phala-proven.
