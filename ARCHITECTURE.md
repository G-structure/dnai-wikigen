...
`c6b446fe9b7ff656c6db799a7461a2a6dba045ae24de16d365c80a6cb83863da`, and
...
checkpoint paths. The current deployed diagnostic receipt classifies the
failure as `invalid_request` / `4xx` from Tinker SDK `0.15.0`; capability probing
did not return a supported-model result through `ServiceClient.get_server_capabilities`.
Follow-up probes using the official quickstart model `Qwen/Qwen3-8B` at ranks
`32` and `16` failed with the same bounded error hash before training creation,
so the current hypothesis is project/account entitlement or service-side account
configuration rather than only the old Llama/rank-4 default.
...
[real]      A production-shaped bounded SDK smoke surface now exists in source.
            ...
            sample text, checkpoint IDs, or raw model output.
...
[partial]   The project-aware smoke diagnostics image is now a verified local
            deploy candidate but is not yet live. GitHub Actions run
            ...
            SPDX SBOM predicates for that digest. The local
            `docker-compose.tinker-funding-validation.phala.yaml` now pins it
            and `verify-compose-hash` computes compose hash
            `d0aea171db3483ae1dae4ff73294548ca5906481166893114ce048688bf2f3f9`.
            It still needs Phala redeploy, live TDX quote verification, on-chain
            compose approval, and a new bounded smoke receipt.
