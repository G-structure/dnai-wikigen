...
[real]      Smoke failure receipts now include bounded SDK diagnostics: an
            ...
            `sdk_diagnostics` for request shape plus capability probing. The
            next diagnostics build also records enum `sdk_error.failure_site`,
            enum `sdk_error.operator_action`, and optional `TINKER_PROJECT_ID`
            configured/hash evidence without returning the raw project id.
