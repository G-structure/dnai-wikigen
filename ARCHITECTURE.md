...
service-side account entitlement. Source has a local follow-up that adds bounded
`client_config` diagnostics and `TINKER_BASE_URL` plumbing, but that follow-up
is not yet rebuilt, redeployed, attested, or approved on-chain.
...
[partial]   The client-configuration diagnostic source follow-up changes the
            ...
            encrypted Phala env selection. Local rendered-compose
            `verify-compose-hash` computes candidate hash
            `cbf788e98d2d562a4cad0407a76fc64add8f5cad1d7ebd9e9f27fc2604950cf8`,
            with `allowed_envs=[]`, but this exact source/compose change is not
            yet rebuilt, GitHub-attested, checked with the Phala raw-compose/
            allowed-env deploy mode, redeployed, TDX-verified, or approved
            on-chain.
